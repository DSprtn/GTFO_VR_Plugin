...
namespace Valve.VR
{
    public class SteamVR_LoadLevel : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if (progressBarEmpty != null && progressBarFull != null)
            {
                ...
                if (progressBarOverlayHandle != OpenVR.k_ulOverlayHandleInvalid)
                {
                    ...
                    // Texture needs to be set every frame after it is updated since SteamVR_Standalone makes a copy internally to a shared texture.
        ...
        IEnumerator LoadLevel()
        {
            ...
            {
                Debug.Log("<b>[SteamVR_Standalone]</b> Launching external application...");
                var applications = OpenVR.Applications;
                if (applications == null)
                {
                    Debug.Log("<b>[SteamVR_Standalone]</b> Failed to get OpenVR.Applications interface!");
                }
                else
                {
                    var workingDirectory = Directory.GetCurrentDirectory();
                    var fullPath = Path.Combine(workingDirectory, internalProcessPath);
                    Debug.Log("<b>[SteamVR_Standalone]</b> LaunchingInternalProcess");
                    Debug.Log("<b>[SteamVR_Standalone]</b> ExternalAppPath = " + internalProcessPath);
                    Debug.Log("<b>[SteamVR_Standalone]</b> FullPath = " + fullPath);
                    Debug.Log("<b>[SteamVR_Standalone]</b> ExternalAppArgs = " + internalProcessArgs);
