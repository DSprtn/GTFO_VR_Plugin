...
namespace Valve.VR.InteractionSystem
{
    ...
    public class ChaperoneInfo : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            if (chaperone == null)
            {
                Debug.LogWarning("<b>[SteamVR_Standalone Interaction]</b> Failed to get IVRChaperone interface.");
                initialized = true;
                yield break;
            }
            ...
            while (true)
            {
                ...
                if (chaperone.GetPlayAreaSize(ref px, ref pz))
                {
                    ...
                    Debug.LogFormat("<b>[SteamVR_Standalone Interaction]</b> ChaperoneInfo initialized. {2} play area {0:0.00}m x {1:0.00}m", px, pz, roomscale ? "Roomscale" : "Standing");
