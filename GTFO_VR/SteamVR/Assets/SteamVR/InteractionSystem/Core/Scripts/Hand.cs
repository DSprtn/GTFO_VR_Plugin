...
{
    //-------------------------------------------------------------------------
    // Links with an appropriate SteamVR_Standalone controller and facilitates
    ...
    public class Hand : MonoBehaviour
    {
        ...
        protected virtual IEnumerator Start()
        {
            ...
            if (!playerInstance)
            {
                Debug.LogError("<b>[SteamVR_Standalone Interaction]</b> No player instance found in Hand Start()", this);
            }

            if (this.gameObject.layer == 0)
                Debug.LogWarning("<b>[SteamVR_Standalone Interaction]</b> Hand is on default layer. This puts unnecessary strain on hover checks as it is always true for hand colliders (which are then ignored).", this);
            else
            ...
            // We are a "no SteamVR_Standalone fallback hand" if we have this camera set
            ...
            //Debug.Log( "<b>[SteamVR_Standalone Interaction]</b> Hand - initializing connection routine" );
        ...
        protected virtual bool CheckHoveringForTransform(Vector3 hoverPosition, float hoverRadius, ref float closestDistance, ref Interactable closestInteractable, Color debugColor)
        {
            ...
            if (numColliding >= ColliderArraySize)
                Debug.LogWarning("<b>[SteamVR_Standalone Interaction]</b> This hand is overlapping the max number of colliders: " + ColliderArraySize + ". Some collisions may be missed. Increase ColliderArraySize on Hand.cs");
        ...
        private void HandDebugLog(string msg)
        {
            if (spewDebugText)
            {
                Debug.Log("<b>[SteamVR_Standalone Interaction]</b> Hand (" + this.name + "): " + msg);
            }
        }
